...
int main(int argc, char **argv) {
    ...
    known = zeroBits(nbits);
    unknown = zeroBits(nbits);
    ...
    showBits(known, out);
    printf("Known:   %s\n", out);
    showBits(unknown, out);
    printf("Unknown: %s\n", out);

    // printf("%d\n", bitsToInt(strToBits("01001")));
    // calculate buckets to be examined
    //int mask =1;
    for (int k = 0; k < pow(2, unkonwnCount); ++k) {
        //Bits bits=zeroBits(nbits);
        int count = 0;
        for (int l = 0; l < nbits; ++l) {
            if (bitIsSet(unknown, l)) {
                if (k >> count++ & 1) {
                    known = setBit(known, l);
                } else {
                    known = unsetBit(known, l);
                }
            }
        }
